...
def generer_historique():
    ...
    print("=" * 60)
    print("INJECTION D'HISTORIQUE (48H)")
    print("=" * 60)
    print("Génération de transactions passées pour initialiser les graphiques...")

    maintenant = datetime.now()
    start_date = maintenant - timedelta(hours=48)
    
    total_envoye = 0
    
    # On parcourt heure par heure
    current_time = start_date
    while current_time < maintenant:
        # Nombre de transactions variable selon l'heure (plus en journée, moins la nuit)
        heure = current_time.hour
        if 0 <= heure < 6:
            nb_trans = random.randint(5, 15)
        elif 6 <= heure < 12:
            nb_trans = random.randint(20, 50)
        elif 12 <= heure < 18:
            nb_trans = random.randint(30, 60)
        else:
            nb_trans = random.randint(20, 40)
        ...
        for _ in range(nb_trans):
            ...
            # Logique transaction (copiée du générateur)
    ...
    print("Le détecteur devrait les traiter sous peu.")
    print("=" * 60)
